...
func limit_b(ctx context.Context, args []string, dev io.Writer) (err error) {
    ...
    const limitTimeout time.Duration = 3
    ctxFire, cancel := context.WithTimeout(context.Background(), limitTimeout * time.Second)
    defer cancel()
    ...
    for _, user := range userRow {
        ...
        if (user.secondUsed + oneDay >= user.secondBase) {
            ...
            waitForTell.Add(1)
            go updateSecondLimit(ctxFire, &waitForTell, chanErr, db, user.username)

            continue
        }

        if (user.bytesUsed + oneGig >= user.bytesBase) {
            if user.bytesLimit == true {
                ob.Fprintln(dev, "notified")
                continue
            }
...
func updateSecondLimit(ctx context.Context,wg *sync.WaitGroup, chanErr chan error, db *sql.DB, username string){
    ...
    err := <-chanErr
    if err != nil {
        config.Log.Error("tell.Fire failed, db.Exec UPDATE ignored", "username", username)
        config.Log.Debug("tell.Fire failed, db.Exec UPDATE ignored", "username", username, "error", err)
    } else {
        _, errExec := db.ExecContext(ctx, `UPDATE bytes SET seconds_limit = true WHERE username = ? AND seconds_limit = false`, username)
        if errExec != nil {
                config.Log.Error("db.ExecContext UPDATE failed", "username", username, "error", errExec)
                err = fmt.Errorf("db.ExecContext UPDATE username %s failed  %w", username, errExec)
                return
        }
...
func updateByteLimit(ctx context.Context,wg *sync.WaitGroup, chanErr chan error, db *sql.DB, username string){
    ...
    if err != nil {
        ...
        _, errExec := db.ExecContext(ctx, `UPDATE bytes SET bytes_limit = true WHERE username = ? AND bytes_limit = false`, username)
        if errExec != nil {
                config.Log.Error("db.ExecContext UPDATE failed", "username", username, "error", errExec)
                err = fmt.Errorf("db.ExecContext UPDATE username %s failed  %w", username, errExec)
                return
